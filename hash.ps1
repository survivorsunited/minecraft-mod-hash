...
function Create-ModsZip {
    ...
    try {
        ...
        # Write README.md to temp directory with zip filename
        $zipReadmeFileName = "$zipFileName.md"
...
$timestamp = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
$filePrefix = "client-mod-all-$combinedHash-$timestamp"
...
$hashFileName = "$filePrefix-hash.txt"
$hashContent | Out-File -FilePath (Join-Path $OutputPath $hashFileName) -Encoding UTF8
...
$readmeContent += "The zip file is named `client-mod-all-<hash>-<timestamp>.zip` and contains all mod JAR files in the root directory."
$readmeContent += ""
$readmeContent += "### Package Documentation"
$readmeContent += "The zip package includes a README file named `client-mod-all-<hash>-<timestamp>.zip.md` with installation instructions and package information."
$readmeContent += ""
...
$readmeFileName = "$filePrefix-README.md"
$readmeContent | Out-File -FilePath (Join-Path $OutputPath $readmeFileName) -Encoding UTF8
...
$zipResult = $null
if ($CreateZip) {
    $zipResult = Create-ModsZip -MandatoryMods $mandatoryMods -SoftWhitelistedMods $softWhitelistedMods -CombinedHash $combinedHash -OutputPath $OutputPath -FilePrefix $filePrefix
}
